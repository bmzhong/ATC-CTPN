...
batch_size=2
python3.7 task_process.py --mode='benchmark.x86_64' --batch_size=${batch_size}
if [ $? != 0 ]; then
    echo "fail!"
    exit -1
fi
...
python3.7 script/script.py -g=script/gt.zip -s=script/predict_txt.zip > data/result_bs${batch_size}.json


echo "====310 accuracy data===="
python3.7 test/parse.py data/result_bs${batch_size}.json
if [ $? != 0 ]; then
    echo "fail!"
    exit -1
fi
